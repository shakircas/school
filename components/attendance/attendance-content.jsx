                ...
                <SelectContent>
                  ...
                  {classes &&
                    classes.map((classItem) =>
                      // Map over each class (e.g., Class 8)
                      classItem.sections.map((sec) => (
                        // Then map over its sections (e.g., Section A)
                        <SelectItem
                          key={`${classItem._id}-${sec._id}`}
                          value={`${classItem.name}-${sec.name}`}
                        >
                          {classItem.name} - {sec.name}{" "}
                          {/* Shows "Class 8 - A" */}
                        </SelectItem>
                      ))
                    )}
...
function AttendanceCard({ student, status, onChange }) {
  ...
  return (
    <Card
      className={`relative overflow-hidden transition-all hover:shadow-md ${config.borderColor} border-l-4`}
    >
