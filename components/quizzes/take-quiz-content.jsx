...
import { Clock, ChevronLeft, ChevronRight, Send, CheckCircle, XCircle } from "lucide-react"
...
export function TakeQuizContent({ quizId }) {
  ...
  const { data: quizData, isLoading } = useSWR(`/api/quizzes/${quizId}`, fetcher)
            ...
            <div className="space-y-4">
              {quiz.questions.map((q, index) => {
                ...
                return (
                  <div
                    key={index}
                    className={`p-4 rounded-lg border ${isCorrect ? "bg-green-50 dark:bg-green-950 border-green-200" : "bg-red-50 dark:bg-red-950 border-red-200"}`}
                  >
                    ...
                    <div className="mt-2 text-sm">
                      ...
                      {!isCorrect && <p className="text-green-600">Correct answer: {q.options[q.correctAnswer]}</p>}
                    </div>
                  </div>
                )
              })}
              ...
              {question.options.map((option, index) => (
                <div
                  ...
                  className={`flex items-center space-x-3 p-4 rounded-lg border cursor-pointer transition-colors ${
                    answers[currentQuestion] === index ? "bg-primary/10 border-primary" : "hover:bg-muted"
                  }`}
