...
export function MCQsContent() {
  ...
  const {
    ...
    defaultValues: {
      ...
      options: ["", "", "", ""],
      correctAnswer: 0,
      class: "1", // Updated default value to be a non-empty string
      subject: "math", // Updated default value to be a non-empty string
      difficulty: "medium",
    },
  })
  ...
  const onSubmit = async (data) => {
    try {
      ...
      toast.success(selectedMCQ ? "MCQ updated successfully" : "MCQ added successfully")
      setIsAddOpen(false)
      setSelectedMCQ(null)
      reset()
      mutate()
  ...
  const filteredMCQs =
    mcqs?.data?.filter((mcq) => {
      if (filterClass && mcq.class !== filterClass) return false
      if (filterSubject && mcq.subject !== filterSubject) return false
      if (searchQuery && !mcq.question.toLowerCase().includes(searchQuery.toLowerCase())) return false
      return true
    }) || []
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="MCQ Question Bank" description="Manage multiple choice questions for practice">
        <div className="flex gap-2">
          ...
          <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>{selectedMCQ ? "Edit Question" : "Add New Question"}</DialogTitle>
                <DialogDescription>
                  {selectedMCQ ? "Update the MCQ question" : "Add a new MCQ to the question bank"}
                </DialogDescription>
              </DialogHeader>
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Button type="button" variant="ghost" size="sm" onClick={handleAIGenerate} disabled={isGenerating}>
                ...
                <div className="space-y-3">
                  ...
                  <RadioGroup
                    ...
                    onValueChange={(value) => setValue("correctAnswer", Number.parseInt(value))}
                  >
                    {[0, 1, 2, 3].map((index) => (
                      <div key={index} className="flex items-center gap-3">
                        <RadioGroupItem value={index.toString()} id={`mcq-opt-${index}`} />
                        <Input
                ...
                <DialogFooter>
                  ...
                  <Button type="submit">{selectedMCQ ? "Update" : "Add Question"}</Button>
                </DialogFooter>
              </form>
            </DialogContent>
          </Dialog>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <Card>
          <CardContent className="pt-6">
            <div className="text-2xl font-bold">{mcqs?.data?.length || 0}</div>
            <p className="text-sm text-muted-foreground">Total Questions</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <div className="text-2xl font-bold">{mcqs?.data?.filter((m) => m.difficulty === "easy").length || 0}</div>
            <p className="text-sm text-muted-foreground">Easy</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <div className="text-2xl font-bold">{mcqs?.data?.filter((m) => m.difficulty === "medium").length || 0}</div>
            <p className="text-sm text-muted-foreground">Medium</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <div className="text-2xl font-bold">{mcqs?.data?.filter((m) => m.difficulty === "hard").length || 0}</div>
                  ...
                  <TableRow key={mcq._id}>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-1">
                        ...
                        <Button variant="ghost" size="icon" onClick={() => handleDelete(mcq._id)}>
