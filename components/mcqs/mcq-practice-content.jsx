...
export function MCQPracticeContent() {
  ...
  const { data: mcqs, isLoading } = useSWR(
    selectedClass && selectedSubject
      ? `/api/mcqs?class=${selectedClass}&subject=${selectedSubject}`
      : null,
    fetcher
  );
  console.log(mcqs);
  ...
  if (completed) {
    ...
    const percentage =
      totalAnswered > 0
        ? ((stats.correct / totalAnswered) * 100).toFixed(0)
        : 0;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="MCQ Practice"
        description={`${selectedSubject} - Class ${selectedClass}`}
      >
              ...
              {currentQuestion.options.map((option, index) => {
                ...
                return (
                  <div
                    ...
                    onClick={() =>
                      !showAnswer && handleAnswer(index.toString())
                    }
                  >
                    <RadioGroupItem
                      value={index.toString()}
                      id={`practice-opt-${index}`}
                      disabled={showAnswer}
                    />
                    <Label
                      htmlFor={`practice-opt-${index}`}
