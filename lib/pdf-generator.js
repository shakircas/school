...
function addHeader(doc, title, subtitle = "") {
  ...
  doc.text(`Phone: ${SCHOOL_INFO.phone} | Email: ${SCHOOL_INFO.email}`, pageWidth / 2, 32, { align: "center" })
...
function addFooter(doc, showPageNumbers = true) {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    // Generated date
    doc.text(`Generated on ${new Date().toLocaleDateString("en-US", { dateStyle: "long" })}`, 15, pageHeight - 10)
    ...
    // Website
    doc.text(SCHOOL_INFO.website, pageWidth / 2, pageHeight - 10, { align: "center" })
  }
}
...
export function generateStudentListPDF(students, options = {}) {
  ...
  const startY = addHeader(doc, "Student List", options.subtitle || `Total Students: ${students.length}`)

  const headers = ["Roll No", "Name", "Class", "Section", "Father Name", "Phone", "Status"]
...
export function generateTeacherListPDF(teachers, options = {}) {
  const doc = new jsPDF(options.orientation || "landscape")
  const startY = addHeader(doc, "Teacher List", options.subtitle || `Total Teachers: ${teachers.length}`)

  const headers = ["ID", "Name", "Designation", "Department", "Phone", "Email", "Status"]
...
export function generateDMCPDF(student, results, examName) {
  ...
  doc.setFont("helvetica", "bold")
  doc.setTextColor(30, 30, 30)
  doc.text("DETAILED MARKS CERTIFICATE", pageWidth / 2, 52, { align: "center" })

  doc.setFontSize(11)
  doc.setFont("helvetica", "normal")
  doc.text(examName || "Annual Examination 2024", pageWidth / 2, 60, { align: "center" })
  ...
  doc.autoTable({
    head: [["#", "Subject", "Total", "Obtained", "Percentage", "Grade", "Remarks"]],
    body: tableData,
    startY: y,
...
export function generateRollNumberSlipPDF(student, exam) {
  ...
  doc.setFont("helvetica", "normal")
  doc.setTextColor(80, 80, 80)
  doc.text(exam.name || "Annual Examination 2024", pageWidth / 2, 46, { align: "center" })
  ...
  doc.setFontSize(8)
  doc.setTextColor(80, 80, 80)
  doc.text("Instructions: Bring this slip daily. No electronic devices allowed.", 20, instrY)
...
export function generateAdmissionListPDF(admissions, options = {}) {
  ...
  const startY = addHeader(doc, "Admission List", options.subtitle || `Academic Year 2024-25`)

  const headers = ["S.No", "Reg. No", "Name", "Father Name", "Class", "Section", "Admission Date", "Previous School"]
...
export function generateFeeReportPDF(feeData, options = {}) {
  const doc = new jsPDF()
  const startY = addHeader(doc, "Fee Collection Report", options.subtitle || options.period || "Monthly Report")

  const headers = ["Roll No", "Name", "Class", "Total Fee", "Paid", "Pending", "Status"]
  ...
  const totalPending = feeData.reduce((sum, f) => sum + (f.pendingAmount || 0), 0)

  doc.setFillColor(245, 247, 250)
  doc.roundedRect(15, summaryY, doc.internal.pageSize.getWidth() - 30, 25, 3, 3, "F")

  doc.setFontSize(11)
  doc.setFont("helvetica", "bold")
  doc.text(`Total Collected: Rs. ${totalCollected.toLocaleString()}`, 25, summaryY + 10)
  doc.text(`Total Pending: Rs. ${totalPending.toLocaleString()}`, 25, summaryY + 20)
